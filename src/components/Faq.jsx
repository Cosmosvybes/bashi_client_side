...
const Faq = () => {
    ...
    const [questions, setQuestion] = useState([
        ...
        },
        {
            ...
            details: 'Sarubashi Token operates on the Binance Smart Chain network and uses a deflationary model. Each transaction incurs a small fee, a portion of which is burned, reducing the total supply of the token over time. Additionally, holders are rewarded with a portion of each transaction fee',
            open: false
        },
        {
            id: 3,
        ...
        {
            id: 7,
            title: 'What charities does Sarubashi Token support?',
        ...
        {
            id: 8,
            title: ' How can I get involved with Sarubashi Token?',
    ...
    const openFaqs = (id) => {
        setQuestion(questions.map((question) => question.id === id ? { ...question, open: !question.open } : question));
        setOpen(!open)
    }
    return (
        <>
            <section id="faqs" style={{ backgroundImage: `url(${bgImage})`, backgroundSize: 'cover', display: 'block', backgroundRepeat: 'no-repeat' }}>
                <div className='faqs-container'>
                    <img src={faq} alt={'faq'} />

                    <div className='faqs'>
                        <h1 style={{ fontSize: '16px', textAlign: 'center', color: 'rgba(89, 205, 153, 0.682)' }}> Frequently Asked Questions </h1>
                        {questions.map((question) =>
                            <div key={question.id} className='faqs'>
                                <button onClick={() => { openFaqs(question.id) }} > {question.title} {question.open ? <FaMinus style={{ float: 'right' }} /> : <FaPlus style={{ float: 'right' }} />} </button>
                                <div style={{ height: question.open && '140px', transition: '0.3s ease-in-out' }} className='answer'><p> {question.details} </p></div>
                            </div>
                        )}
                    </div>
