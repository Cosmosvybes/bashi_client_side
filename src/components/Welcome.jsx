  ...
  const sendETH = async (e) => {
    ...
    if (canBuy) {
      try {
        ...
        setReceipt(true);
        setHash(hash.hash);
        fetch(`https://sarubashi-app.onrender.com/buytoken`, {
          method: "POST",
          mode: "cors",
          headers: { "Content-Type": "Application/json" },
          body: JSON.stringify({ wallet: address, amount: qty }),
        })
          .then((response) => response.json())
          .then((res) => {
            setBal(res.balance);
    ...
    <>
      <section className="coin-details">
        <div className="headline">
          ...
          <h1 style={{ color: "grey" }}>
            {" "}
            Is 100% secure token, embracing the wisdom of its namesake.{" "}
          </h1>
          <p style={{ color: "rgb(89,205,153,0.682)" }}>
            this multi-purpose token is committed to providing a secure ,
            reliable & trustworthy blockchain{" "}
          </p>
          <h1 style={{ color: "rgb(89,205,153,0.682)" }}>
            {" "}
            Powerful and fast for its vibrant community
          </h1>
          <Button
            value="KYC certificate"
            color="lightgrey"
            backgorund={"rgb(89,205,153,0.682)"}
            width={"125px"}
            callback={showKyc}
          />
        </div>
        ...
        <div className="tokendetails">
          ...
          <div className="buy-earn-btn">
            <Button
              value={!buybtn ? "BUY $BASHI" : "close"}
              color={"lightgrey"}
              backgorund={!buybtn ? "rgb(89,205,153,0.682)" : "red"}
              callback={showWalletConnect}
            />{" "}
            <Link
              to={"referral"}
              smooth={true}
              ...
              style={{
                textDecoration: "none",
                backgroundColor: "rgb(89,205,153,0.682)",
                height: "40px",
                padding: "10px 10px",
                color: "lightgrey",
                border: "1px solid rgb(89,205,153,0.682)",
          ...
          <div className="modal-reciept-container">
            <div className="buying-modal">
              <form onSubmit={sendETH}>
                <label>
                  <b style={{ textAlign: "center", color: "lightgrey" }}>
                    {" "}
                    $BASHI wallet will be generated soon as you purchase your
                    your first token{" "}
                  </b>
                </label>
                <label>
                  <input
                    ...
                    style={{
                      border: "1px solid rgba(89, 205, 153, 0.682) ",
                      color: "green",
                    }}
                    step="0.01"
                    required
                ...
                <input
                  ...
                  style={{
                    color: "lightgrey",
                    backgroundColor: " rgba(89, 205, 153, 0.682)",
                    border: "1px solid  rgba(89, 205, 153, 0.682)",
                  }}
                />
                <label>
            ...
            {receipt && (
              <div className="wallet-connect_">
                <p> Purchase reciept ; </p>
                <p>
                  {" "}
                  Hurray🎊🎉 you have successfully purchase{" "}
                  <span style={{ color: "green" }}> {qty}</span> sarubashi token
                  🤑{" "}
                </p>
